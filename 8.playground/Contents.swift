...
extension String {
    var integerComponents: [Int] {
        let stringToParse = self.replacingOccurrences(of: "-", with: "")
        
        let zeroCharacterValue = "0".unicodeScalars.first!.value
        var components = [Int]()
        
        for i in stringToParse.unicodeScalars {
            let characterValue = i.value
            let integer = characterValue - zeroCharacterValue
    ...
    var integerValue: Int {
        let components = integerComponents
        var int = 0
        
        for i in 0..<components.count {
            let component = components[i]
            let powerOf10 = components.count-1 - i
            int += component * Int(pow(10, Double(powerOf10)))
        }
...
struct Command {
    ...
    init(string: String) {
        ...
        self.operation = Operation(string: components[1])
        self.register = components[0]
        self.value = components[2].integerValue
    }
}
...
struct Condition {
    enum Comparator {
        ...
        func registerWithValueSatifies(registerValue: Int, value: Int) -> Bool {
            switch self {
            case .greaterThan:
    ...
    init(string: String) {
        let components = string.components(separatedBy: " ")
        self.register = components[0]
        self.comparator = Comparator(string: components[1])
        self.value = components[2].integerValue
    }
    
    func registerSatisfies(_ registerValue: Int) -> Bool {
        return comparator.registerWithValueSatifies(registerValue: registerValue, value: value)
    }
}
...
struct Instruction {
    ...
    init(line: String) {
        // Split by "if" to find the two strings
        let components = line.components(separatedBy: " if ")
        self.command = Command(string: components[0])
        self.condition = Condition(string: components[1])
    }
}

func instructionsFromInput(_ input: String) -> [Instruction] {
    var instructions = [Instruction]()
    for line in input.components(separatedBy: "\n") {
        instructions.append(Instruction(line: line))
    }
    return instructions
}
...
struct RegisterSet {
    ...
    mutating func performCommand(_ command: Command) {
        if !backingDictionary.keys.contains(command.register) {
            backingDictionary[command.register] = 0
        }
    ...
    mutating func performInstruction(_ instruction: Instruction) {
        if instruction.condition.registerSatisfies(valueForRegister(instruction.condition.register)) {
            performCommand(instruction.command)
        }
    }
    ...
    var largestRegisterValue: Int {
        var largestRegisterValue = Int.min
        for register in backingDictionary.keys {
            largestRegisterValue = max(largestRegisterValue, valueForRegister(register))
        }
...
let sampleInstructions = instructionsFromInput(sampleInput)
var sampleRegisterSet = RegisterSet()
