...
func checksumForSpreadsheet(_ spreadsheet: String) -> Int {
    ...
    func intRowForStringRow(_ stringRow: String) -> [Int] {
        ...
        let strings = stringRow.split(separator: " ")
        for string in strings {
            intRow.append(String(string).integerValue)
        }
        
        return intRow
    }
    
    let stringRows = spreadsheet.split(separator: Character("\n"))
    var rows = [[Int]]()
    ...
    for row in rows {
        let rowChecksum = checksumForRow(row)
