...
func nodeForLine(_ line: String) -> Node {
    let components = line.components(separatedBy: "->")
    let nameAndWeightComponent = components.first!
    let name = nameAndWeightComponent.components(separatedBy: " ").first!
    let weightWithParens = nameAndWeightComponent.components(separatedBy: " ")[1]
    var weightWithoutParens = weightWithParens.replacingOccurrences(of: "(", with: "")
    weightWithoutParens = weightWithoutParens.replacingOccurrences(of: ")", with: "")
    let weight = weightWithoutParens.integerValue
    
    let childrenNames: [String]
    if components.count > 1 {
        var childrenComponent = components.last!
        childrenComponent = childrenComponent.replacingOccurrences(of: " ", with: "")
        childrenNames = childrenComponent.components(separatedBy: ",")
    }
    else {
        childrenNames = [String]()
    }
    
    return Node(name: name, weight: weight, childrenNames: childrenNames, parentName: nil)
}

func nodesForInput(_ input: String) -> [Node] {
    var nodes = [Node]()
    
    for line in input.components(separatedBy: CharacterSet.newlines) {
        let node = nodeForLine(line)
        nodes.append(node)
    }
    
    // Now that we've loaded all nodes, we need to set up the parent pointers
    // for all of them
    for node in nodes {
        for childName in node.childrenNames {
            if let child = nodes.nodeWithName(childName) {
                let childIndex = nodes.index(of: child)!
                nodes[childIndex].parentName = node.name
            }
        }
    }
...
let sampleNodes = nodesForInput(sampleInput)
